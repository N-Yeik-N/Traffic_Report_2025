...
def get_sigs_propuesto(subareaPath) -> None:
    ...
    pattern = r"([A-Z]+-[0-9]+)"
    actualPath = os.path.join(subareaPath,"Propuesto - 1 año")
    ...
    for text, pathImg, tipicidad, scenarioName, code in listData:
        ...
        finalPath = os.path.join(imagesDirectory, f"SP1Y_{code}_{unidecode(tipicidad).upper()}_{scenarioName}.docx")
        doc_template.save(finalPath)
        listWordPaths.append(finalPath)
    
    sigsProposed1Y = os.path.join(subareaPath, "Tablas", "SigProposed1Y.docx")
    filePathMaster = listWordPaths[0]
    filePathList = listWordPaths[1:]

    _combine_all_docx(filePathMaster, filePathList, sigsProposed1Y)

    return sigsProposed1Y