...
def peakhour_finder_and_volumes(path) -> Hours:
    ...
    SUM_QUARTER = [row[0].value for row in ws[slice("HQ16", "HQ111")]]
    
    wb.close()

    peakhour_morning = max(SUM_HOUR[6*4:12*4])
    index_morning = SUM_HOUR[6*4:12*4].index(peakhour_morning) + 6*4-3
    volume_morning = SUM_HOUR[index_morning+3]
    
    peakhour_evening = max(SUM_HOUR[12*4:17*4])
    index_evening = SUM_HOUR[12*4:17*4].index(peakhour_evening) + 12*4-3
    volume_evening = SUM_HOUR[index_evening+3]

    peakhour_night = max(SUM_HOUR[17*4:24*4])
    index_night = SUM_HOUR[17*4:24*4].index(peakhour_night) + 17*4-3
    ...
    return excel_hours_info, SUM_QUARTER
